...
$(document).ready(function() {
     $('.jedit').editable('jeditablenew.php', { 
         type      : 'textarea',
         cancel    : '取消',
         submit    : '修改',
         indicator : '正在儲存中',
 ...
 $('.edit').editable('jeditablenew.php',{
     data   : "{'unknown':'不明','scholarship':'學術關係','friendship':'朋友關係', 'politics':'政治關係','writing':'著述關係', 'military':'軍事關係', 'medicine':'醫療關係','religion':'宗教關係', 'family':'家庭關係', 'finance':'財務關係','norelation':'無關聯',}",
     type   : 'select',
     submit : 'OK'
     });  
});
...
<form class="ui form" method="GET" action="index.php">
	<div class="ui segment">
		...
		<div class="field">
         ...
         <input type="checkbox" name="relation" value="writing"> 著述關係<br>
          <input type="checkbox" name="relation" value="military">軍事關係<br>
          <input type="checkbox" name="relation" value="medicine">醫療關係<br>
          <input type="checkbox" name="relation" value="religion">宗教關係<br>
          <input type="checkbox" name="relation" value="family">家庭關係<br>
          <input type="checkbox" name="relation" value="finance">財務關係<br>
          <input type="checkbox" name="relation" value="unknown">不明<br>
           <input type="checkbox" name="relation" value="norelation">無關聯<br>			
		</div>
...
if (isset($search_name) && $search_name !== "") {
	array_push($cond, 'name LIKE :name OR targetname LIKE :targetname ');
	$values[":name"] = '%' . $search_name . '%';
	$values[":targetname"] = '%' . $search_name . '%';
}


# OR targetid  LIKE :targetid

if (isset($search_cbdbid) && $search_cbdbid !== "") {
	array_push($cond, 'idname LIKE :idname  OR targetid  LIKE :targetid');
	$values[":idname"] = '%' .$search_cbdbid . '%';
	$values[":targetid"] = '%' .$search_cbdbid . '%';
...
$social_network2s = R::getAll("SELECT * FROM social_network2 WHERE " . implode(" AND ", $cond) . " ORDER BY id, title", $values);
...
//echo "SELECT id, title, name, date, fulltext FROM social_network2 WHERE " //. implode(" ", $cond) ;
...
$chapter = '<a href="index.php?social_network2_id='.$id . '" target="_blank">' . $chapter . '</a>';
	echo '<td class="top aligned" title="'.$id.'">' . $chapter . '</td>';
	
     $idname = $social_network2->idname;
	echo '<td class="top aligned jedit" title="' . $idname . '"  id= " idname_'.$id.' ">' . $idname.'</td>';
	...
	echo '<td class="top aligned jedit" title="' . $ori_name . ' " id=  "name_'.$id.' ">' . $name .'</td>';

     $targetid = $social_network2->targetid;
	echo '<td class="top aligned jedit" title=" '. $targetid .'   "  id= "  targetid_ '.$id.' ">'.$targetid.'</td>';
	...
	echo '<td class="top aligned jedit" title=" ' . $ori_targetname . ' " id= "  targetname_ '.$id.' ">' . $targetname . '</td>';
	//echo '<td>' . $social_network2->id . '</td>';
	
	// -----------------------
	$relation = $social_network2->relation;
	echo '<td class="top aligned edit" title="'.$relation.'"  id= "relation_'.$id.' " >' . $relation . '</td>';
